---
title: "9. Dubbo3 服务调用源码分析"
date: 2025-07-19 18:50:00 +0800
categories: [从零开始的Dubbo源码阅读]
tags: [dubbo, 源码阅读]
---
# 服务调用过程

> 参考链接：[dubbo2.6源码分析](https://cn.dubbo.apache.org/zh-cn/docsv2.7/dev/source/service-invoking-process/)
...
![img](https://cn.dubbo.apache.org/imgs/dev/send-request-process.jpg)
...
public class InvokerInvocationHandler implements InvocationHandler {
    ...
    /**
     * 代理对象的方法调用处理器，实现InvocationHandler接口
     * 处理代理对象上的所有方法调用
     */
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        // 如果调用的是Object类的方法（如wait, notify等），直接调用原始对象的对应方法
        ...
        if (parameterTypes.length == 0) {
            // 处理toString方法，直接返回原始对象的toString结果
            if ("toString".equals(methodName)) {
                return invoker.toString();
            } 
            // 处理$destroy方法，调用原始对象的destroy方法进行资源销毁
            else if ("$destroy".equals(methodName)) {
                invoker.destroy();
                return null;
            } 
            // 处理hashCode方法，直接返回原始对象的hashCode
        ...
        RpcInvocation rpcInvocation = new RpcInvocation(
                serviceModel,                   // 服务模型，包含服务相关元数据
                method.getName(),               // 方法名
                invoker.getInterface().getName(), // 接口名
                protocolServiceKey,             // 协议服务键
                method.getParameterTypes(),     // 参数类型
                args                            // 参数值
        );

        // 如果是消费者模型，添加额外的消费者和方法模型信息
        if (serviceModel instanceof ConsumerModel) {
            rpcInvocation.put(Constants.CONSUMER_MODEL, serviceModel);
            rpcInvocation.put(Constants.METHOD_MODEL, ((ConsumerModel) serviceModel).getMethodModel(method));
        }

        // 通过InvocationUtil工具类调用实际的RPC方法并返回结果
...
public class MockClusterInvoker<T> implements Invoker<T> {
    ...
    /**
     * 执行RPC调用并根据mock配置处理结果
     * 
     * @param invocation 调用上下文信息
     * @return 调用结果
     * @throws RpcException 当发生RPC异常时抛出
     */
    public Result invoke(Invocation invocation) throws RpcException {
        Result result;

        // 从URL中获取当前方法的mock配置，默认为不启用
        String value = getUrl().getMethodParameter(
                        RpcUtils.getMethodName(invocation), MOCK_KEY, Boolean.FALSE.toString())
                .trim();

        // 情况1：无mock配置，直接执行远程调用
        if (ConfigUtils.isEmpty(value)) {
            result = this.invoker.invoke(invocation);
        } 
        // 情况2：强制mock模式，无论远程调用是否成功都使用mock逻辑
        ...
        // 情况3：失败降级模式，仅在远程调用失败时使用mock逻辑
        else {
            try {
                // 尝试执行远程调用
                result = this.invoker.invoke(invocation);

                // 处理已返回但包含异常的情况（非业务异常时降级）
                if (result.getException() != null && result.getException() instanceof RpcException) {
                    RpcException rpcException = (RpcException) result.getException();
                    if (rpcException.isBiz()) { // 业务异常直接抛出
                        throw rpcException;
                    } else { // 非业务异常（如网络异常）执行降级
...
public abstract class AbstractInvoker<T> implements Invoker<T> {
    ...
    /**
     * 处理RPC调用的主方法，包含前置检查、异步结果处理和同步等待逻辑
     */
    public Result invoke(Invocation inv) throws RpcException {
        // 允许已销毁的Invoker完成当前调用（避免地址刷新导致的请求中断）
        if (isDestroyed()) {
            logger.warn("");
        }
...
![img](https://cn.dubbo.apache.org/imgs/dev/send-request-thread-stack.jpg)
...
![img](https://cn.dubbo.apache.org/imgs/dev/data-format.jpg)
...
![img](https://cn.dubbo.apache.org/imgs/dev/dispatcher-location.jpg)
...
![img](https://cn.dubbo.apache.org/imgs/dev/request-id-application.jpg)
